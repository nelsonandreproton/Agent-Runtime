...
- **Markdown Agent Loader** (`loader.py`) — parses agent `.md` files in the exact frontmatter format used by Claude Code / Cowork subagents (`name`, `description`, `tools`, `model` + a Markdown body used as the system prompt). Seven real examples are bundled in `agents/`, including a handful shaped around ODC/Salesforce call patterns: `contract-reviewer`, `support-ticket-triage`, `sales-followup-drafter`, `meeting-notes-summarizer`, `knowledge-base-assistant`, plus `code-reviewer` and `security-reviewer`.
...
- For a stable long-term URL, either use a [named Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/get-started/create-remote-tunnel/) tied to a domain you control, or move to a real reverse-proxy deployment (see "Deploy to Hetzner" patterns used by sibling A2A projects in this workspace).
