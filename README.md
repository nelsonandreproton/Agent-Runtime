...
**Windows, one command:** `scripts/run_all.ps1` starts llama.cpp, a static-domain ngrok tunnel, and the gateway together — each in its own window, waiting for each to become ready before starting the next. It's a template with your own setup's constants at the top (model, ngrok domain, llama.cpp path) rather than a generic script, since it hardcodes one specific local stack; edit those constants before running. Requires an ngrok [reserved/static domain](https://ngrok.com/docs/universal-gateway/domains/) (free tier includes one) so `AGENT_RUNTIME_PUBLIC_URL` never goes stale across restarts — with a random quick-tunnel URL you'd need to update `.env` and restart the gateway every time the tunnel restarts (see the `AGENT_RUNTIME_PUBLIC_URL` note below).
